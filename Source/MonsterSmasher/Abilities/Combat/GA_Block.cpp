...
UGA_Block::UGA_Block()
{
	...
	SetAssetTags(FGameplayTagContainer(TAG_InputTag_Block));
                                ...
                                const FGameplayEventData* TriggerEventData)
{
	...
	if (BlockingStanceGE && ActorInfo->AbilitySystemComponent.IsValid())
	{
		...
		FGameplayEffectSpecHandle SpecHandle = ASC->
			MakeOutgoingSpec(BlockingStanceGE, GetAbilityLevel(), EffectContext);
                           ...
                           bool bWasCancelled)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UGA_Block: EndAbility called. Was Cancelled: %s"),
